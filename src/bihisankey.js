...
export default function biHiSankey() {
    "use strict";
    ...
    function computeNodeValues() {
        nodes.forEach(function (node) {
            ...
            node.netFlow = sum(visible(node.targetLinks), value) - sum(visible(node.sourceLinks), value);
    ...
    function scaleNodeXPositions() {
        var minX = min(nodes, function (node) { return node.x; }),
            maxX = max(nodes, function (node) { return node.x; }) - minX;
    ...
    function computeNodeYPositions(iterations) {
        ...
        function calculateYScaleFactor() {
            ...
            yScaleFactor = min(nodesByXPosition, function (nodes) {
                linkSpacesCount = sum(nodes, function (node) {
                    return node.linkSpaceCount;
                });
                nodeValueSum = sum(nodes, function (node) {
                    return node.value;
                });
            ...
            nodesByXPosition.forEach(function (nodes) {
                nodes.forEach(function (node) {
                    if (node.rightLinks.length) {
                        var y = sum(node.rightLinks, weightedSource) / sum(node.rightLinks, value);
            ...
            nodesByXPosition.slice().reverse().forEach(function (nodes) {
                nodes.forEach(function (node) {
                    if (node.leftLinks.length) {
                        var y = sum(node.leftLinks, weightedTarget) / sum(node.leftLinks, value);
                        node.y += (y - center(node)) * alpha;
                    }
                });
            });
        ...
        minY = min(nodes, function (node) { return node.y; });
        adjustTop(minY);
    }
